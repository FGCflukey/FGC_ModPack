...
local function breakSpear(grave, grave2, spears, spearIndex)
	...
	if not SandboxVars.SpearTraps.SpearsBreak then
		print("Keyword: spears don't break")
		return
	end
...
local function onPlayerUpdate(player)
	...
	if not player:isAlive() or player:isGodMod() or player:isInvincible() then return end

	local vehicle = player:getVehicle()
	if vehicle and SandboxVars.SpearTraps.SpearsDamageVehicles then return updateVehicle(vehicle) end
	...
	if grave ~= nil and not isFilledGrave(grave) then
		...
		if #spears > 0 and  spearIndex > 0 and not pData.onGrave then
			...
			if SandboxVars.SpearTraps.SpearTrapsKillPlayer then
				--print("keyword: killing player")
				player:Kill(nil)
				removeSpearTile(grave)
				breakSpear(grave, grave2, spears, spearIndex)
				
			elseif SandboxVars.SpearTraps.SpearTrapsHurtPlayer then
				--print("keyword: hurting player")
				for i=0, #spears do
		...
		elseif not pData.onGrave then
			...
			if player:isRunning() and SandboxVars.SpearTraps.SpearTrapsHurtPlayer then
				--print("keyword: hurting player on entering trap")
				giveRandomInjury(player)
			end
...
local function onZombieUpdate(zombie)
	...
	if grave ~= nil and not isFilledGrave(grave) then
		...
		if #spears > 0 and  spearIndex > 0 and not zData.onGrave then
			...
			--killZombie(zombie)
			zombie:Kill(nil)
			if SandboxVars.SpearTraps.SpearsBreak then
				removeSpearTile(grave)
				breakSpear(grave, grave2, spears, spearIndex)
			end
