...
SUPEquipWeaponAction = ISBaseTimedAction:derive("SUPEquipWeaponAction");

function SUPEquipWeaponAction:isValid()
	return self.character:getInventory():contains(self.item);
end

function SUPEquipWeaponAction:update()
	self.item:setJobDelta(self:getJobDelta());
end

function SUPEquipWeaponAction:start()
...
function SUPEquipWeaponAction:animEvent(event, parameter)
...
function SUPEquipWeaponAction:stop()
	if self.sound then
...
function SUPEquipWeaponAction:isAlreadyEquipped()
...
function SUPEquipWeaponAction:perform()
	if self.sound then
		self.character:getEmitter():stopSound(self.sound)
	end
	...
	if self.capture == "Diana" then
		--no hacer nada!
		LGHuntingRocksMenu.PracticeHunting(self.square, self.plnum, self.capture)
	else
...
function SUPEquipWeaponAction:new (character, item, time, primary, twoHands, square, capture)
