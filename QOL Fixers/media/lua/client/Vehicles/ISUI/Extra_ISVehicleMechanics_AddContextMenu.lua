...
local function Extra_predicateBlowTorch(item)
...
function ISVehicleMechanics:doPartContextMenu(part, x,y)
	...
	if part:getId() == "Heater" then
		...
		if primaryHandItem ~= nil and primaryHandItem:getType() == "Base.BlowTorch" and primaryHandItem:getDrainableUsesInt() >= 10 then
			...
			blowtorch = playerObj:getInventory():getFirstEvalRecurse(Extra_predicateBlowTorch)
		...
		if currentCondition < 100 and allPartsPresent and self.chr:getPerkLevel(Perks.Mechanics) >= requiredMechanicsSkillLevel and self.chr:getPerkLevel(Perks.MetalWelding) >= requiredMetalworkingSkillLevel and blowtorch ~= nil and mask ~= nil then
			option = self.context:addOption(getText("ContextMenu_Repair"), playerObj, ISVehicleMechanics.Extra_onRepairHeater, part, requiredParts, blowtorch, mask, targetCondition, requiredMechanicsSkillLevel, requiredMetalworkingSkillLevel)
			self:Extra_doMenuTooltip(part, option, "Extra_repairheater", requiredParts, blowtorch, mask, requiredMechanicsSkillLevel, requiredMetalworkingSkillLevel, targetCondition)
		else
...
function ISVehicleMechanics:Extra_doMenuTooltip(part, option, lua, requiredParts, blowtorch, mask, requiredMechanicsSkillLevel, requiredMetalworkingSkillLevel, targetCondition)
	...
	if lua == "Extra_repairheater" then
		local rgb = " <RGB:1,1,1>"
		local addedTxt = ""
		...
		if option.notAvailable then
			tooltip.description = tooltip.description .. " <LINE><RGB:1,0,0>" .. getText("Tooltip_Extra_NewCondition") .. ": " .. targetCondition .. "%"
		else
			tooltip.description = tooltip.description .. " <LINE><RGB:1,1,1>" .. getText("Tooltip_Extra_NewCondition") .. ": " .. targetCondition .. "%"
		end
	end
end

function ISVehicleMechanics.Extra_onRepairHeater(playerObj, part, requiredParts, blowtorch, mask, targetCondition, requiredMechanicsSkillLevel, requiredMetalworkingSkillLevel)
	...
	ISTimedActionQueue.add(ExtraRepairHeater:new(playerObj, part, blowtorch, mask, timeToRepair, requiredParts, targetCondition))
end