...
BVRRepairHeater = ISBaseTimedAction:derive("BVRRepairHeater")

function BVRRepairHeater:isValid()
...
function BVRRepairHeater:waitToStart()
	self.character:faceThisObject(self.vehicle)
	return self.character:shouldBeTurning()
end

function BVRRepairHeater:update()
	self.character:faceThisObject(self.vehicle)
...
function BVRRepairHeater:start()
...
function BVRRepairHeater:perform()
    -- Stop the sound of the blowtorch once the action is complete
    if self.sound then
        self.character:getEmitter():stopSound(self.sound)
        self.sound = nil
	...
	sendClientCommand(self.character, 'BVR_vehicle', 'repairHeater', { vehicle = self.part:getVehicle():getId(), part = self.part:getId(), targetCondition = self.targetCondition, repairParts = self.requiredParts })

	ISBaseTimedAction.perform(self)
end

function BVRRepairHeater:new(character, part, blowtorch, mask, timeToRepair, requiredParts, targetCondition)
	local o = {}
	setmetatable(o, self)
	self.__index = self
