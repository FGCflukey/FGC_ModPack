	...
	if addFuel then
		...
		if #pourOut > 0 then
			...
			for _,item in ipairs(pourOut) do
				subMenu:addOption(item:getName(), addFuel, TRFB_Menu.onAddFuel, player, addFuel:getSquare(), item);
			end
		end
    end
...
TRFB_Menu.onAddFuel = function(fuelBarrel, player, square, fuelItem)
	local barrel = fuelBarrel;

	print("Name:", fuelBarrel:getName());
	local playerObj = getSpecificPlayer(player);
	if not fuelBarrel:getName() or fuelBarrel:getName() == "" then
		--if not isClient() then
			local square = fuelBarrel:getSquare();
			
			FuelBarrel = playerObj:getInventory():AddItem("TRPack.PetrolBarrel");
			barrel = ISFuelBarrel:new("trpack_1", FuelBarrel);
			barrel.player = player;
			barrel.modData["need:TRPack.PetrolBarrel"] = "1";
			-- print("Player:", player);		
			-- print("Square:", square);		
			-- print("FuelBarrel:", FuelBarrel.sprite);	
			-- print("fuelBarrel:", fuelBarrel);	
			-- print("barrel:", barrel);	
			--square:AddTileObject(fuelBarrel);
			barrel:removeOldBarrel(square, fuelBarrel);
			
			barrel:create(square:getX(), square:getY(), square:getZ(), false, "trpack_1");
			for i=0,square:getObjects():size()-1 do
				local checkObject = square:getObjects():get(i);
				if checkObject:getName() == "FuelBarrel" then 
					barrel = checkObject;
					break;
				end
			end
			
			--playerObj:getInventory():Remove(FuelBarrel);
		--end
	end
	print("barrel:", barrel);
	--print("Name:", barrel:getName());
	--false if add fuel to barrel
    if fuelItem and luautils.walkAdj(playerObj, square) then
		if fuelItem:getType() == "PetrolCan" then
			ISTimedActionQueue.add(ISEquipWeaponAction:new(playerObj, fuelItem, 40, true))
		else
			ISTimedActionQueue.add(ISEquipWeaponAction:new(playerObj, fuelItem, 80, true, true))
		end
		print("barrel:", barrel);
		local action = ISFuelBarrelActions:new(false, barrel, playerObj, fuelItem, 40);
		print("actions: ",action.character:getCharacterActions(), " action ", action.action, " V:", action.character:getCharacterActions():contains(action.action));
...
TRFB_Menu.onTakeFuel = function(fuelBarrel, player, square, fuelItem)
    ...
    if fuelItem and luautils.walkAdj(player, square) then
		...
		print("fuelItem:", fuelItem);
		local action = ISFuelBarrelActions:new(true, fuelBarrel, player, fuelItem, 40);
		print("actions: ",action.character:getCharacterActions(), " action ", action.action, " V:", action.character:getCharacterActions():contains(action.action));
		ISTimedActionQueue.add(action);
    end
end
