...
--                getSoundManager():PlayWorldSoundWav("PZ_AnimalTrappedMetal_0" .. ZombRand(3)+1, square, 0, 15, 2, true);
			elseif self.trapType == "Base.BearTrap" then
				local sound = "AnimalInMetalTrap";
				if self.animal ~= nil then
					if self.animal.type == "bear" then
						sound = "TrapBearDie";
					elseif self.animal.type == "chancho" then
						sound = "TrapBoarDie";
					elseif self.animal.type == "vaca" then
						sound = "TrapCowDie";
					end
				end
                square:playSound(sound, true);
            end
        end
    end
end
...
--        print("remove animal");
        ...
        local number = ZombRand(self.animal.minSize, self.animal.maxSize);
		local perklvl = character:getPerkLevel(Perks.Trapping)
		local plpercent = perklvl / 10;
		if plpercent < 0 then
			plpercent = 0.1;
		end
		if (number / self.animal.maxSize) > plpercent then
			if ZombRand(100) > (plpercent * 100) then
				local valuemax = 0;
				if perklvl <= 1 then
					valuemax = 0.6;
				elseif perklvl <= 3 then
					valuemax = 0.5;
				elseif perklvl <= 5 then
					valuemax = 0.4;
				elseif perklvl <= 7 then
					valuemax = 0.3;
				elseif perklvl <= 8 then
					valuemax = 0.2;
				elseif perklvl <= 9 then
					valuemax = 0.1;
				else
				end				
				local newmaxsize = self.animal.maxSize - math.ceil(self.animal.maxSize * valuemax);
				if newmaxsize < self.animal.minSize then
					newmaxsize = self.animal.minSize + 5;
				end
				number = ZombRand(self.animal.minSize, newmaxsize)
			end
		end
		local weight = nil;
		if self.animal.weightChange or self.animal.weightChange ~= nil then
			weight = number / self.animal.weightChange;
		end
        local xp = number / 3;
        if xp < 3 then
            xp = 3;
        ...
        item:setBaseHunger(number);
        item:setHungChange(item:getBaseHunger());
		if self.animal.weightChange or self.animal.weightChange ~= nil then
			local calc = tonumber(string.format("%." .. (2) .. "f", (weight * 2.2)))
			item:setActualWeight(calc);
			item:setCustomWeight(true)
		end
		if self.animal.maxSize then
			if self.animal.maxSize > size then
				local percent = size / self.animal.maxSize;
				item:setLipids(self:calcnut(item:getLipids(), percent));
				item:setProteins(self:calcnut(item:getProteins(), percent));
				item:setCalories(self:calcnut(item:getCalories(), percent));
				item:setCarbohydrates(self:calcnut(item:getCarbohydrates(), percent));
			end
		end

		if item:getType() == "ChickenDead" then
			if ZombRand(2) == 0 then
				local item2 = InventoryItemFactory.CreateItem("Base.Egg")
				for i=1, ZombRand(1, 4) do
					if isServer() then
						character:sendObjectChange('addItem', { item = item2 })
					else
...
------------------------------ Contenido del Mod ---------------------------------
function STrapGlobalObject:calcnut(nv, pct)
	local value = tonumber(string.format("%." .. (2) .. "f", (nv * pct)))
	return value;
end
----------------------------------------------------------------------------------
