...
function OnDepostarAction:start()
	...
	elseif self.animal:getWorldItem() then
		if self.animal:getWorldItem():getSquare() then
			self.character:faceLocation(self.animal:getWorldItem():getX(), self.animal:getWorldItem():getY());
		end
	end
...
function OnDepostarAction:stop()
...
function OnDepostarAction:chop()
    self.character:getStats():setEndurance(self.character:getStats():getEndurance() - 0.01)
...
function OnDepostarAction:addOrDropItems(type, count)
    ...
    for i=1,items:size() do
        if instanceof(items:get(i-1), "Food") then
			...
			if self.animal:getAge() > self.animal:getScriptItem():getDaysTotallyRotten() then
    ...
    for i=1,items:size() do
		...
		if self.square ~= nil then
			if self.square:getSquare() then
				self.square:getSquare():AddWorldInventoryItem(items:get(i-1), 0.0, 0.0, 0.0)
				addBloodSplat(self.square:getSquare(), ZombRand(7, 12))
			end
		elseif self.animal:getWorldItem() then
			if self.animal:getWorldItem():getSquare() then
				self.animal:getWorldItem():getSquare():AddWorldInventoryItem(items:get(i-1), 0.0, 0.0, 0.0)
				addBloodSplat(self.animal:getWorldItem():getSquare(), ZombRand(7, 12))
			end
		end
...
function OnDepostarAction:getMeat()
    ...
    for i,v in ipairs(DepostarDefinitions[self.animal:getType()].items) do
		if not data[v.DataName] then
			data[v.DataName] = v.maxCount;
		end
...
function OnDepostarAction:RemoveAnimal(possibilities, head, skin)
	if head and head <= 0 and skin and skin <= 0 and possibilities <=0 then
		if self.square ~= nil then
			...
			self.square:getSquare():transmitRemoveItemFromSquare(self.square);
			...
			for i=1,20 do
				addBloodSplat(self.animal:getWorldItem():getSquare(), ZombRand(7, 15))
			end
			self.animal:getWorldItem():getSquare():transmitRemoveItemFromSquare(self.animal:getWorldItem());
			self.animal:getWorldItem():getSquare():removeWorldObject(self.animal:getWorldItem());
