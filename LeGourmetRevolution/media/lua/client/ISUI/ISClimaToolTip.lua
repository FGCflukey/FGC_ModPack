...
ISClimaToolTip = ISPanel:derive("ISClimaToolTip");
...
function ISClimaToolTip:initialise()
	ISPanel.initialise(self);
end

function ISClimaToolTip:setName(name)
	self.name = name;
end

function ISClimaToolTip:setContextMenu(contextMenu)
	self.contextMenu = contextMenu;
end

function ISClimaToolTip:setTexture(textureName)
	self.texture = getTexture(textureName);
end

function ISClimaToolTip:onMouseDown(x, y)
	return false
end

function ISClimaToolTip:onMouseUp(x, y)
	return false
end

function ISClimaToolTip:onRightMouseDown(x, y)
	return false
end

function ISClimaToolTip:onRightMouseUp(x, y)
	return false
end

function ISClimaToolTip:prerender()
...
--**
--************************************************************************--
function ISClimaToolTip:render()
...
function ISClimaToolTip:doLayout()
	local textX = 10
	local textY = 0
	local textWidth = 0
	local textHeight = 0
...
function ISClimaToolTip:layoutContents(x, y)
	local textWidth = 0
	local textHeight = 0
	...
	if self.description ~= "" then
		...
		if self.maxLineWidth then
			...
			self.descriptionPanel:setWidth(200 * widthScale)
		end
...
function ISClimaToolTip:renderContents()
	if self.description ~= "" then
		if self.texture then
...
function ISClimaToolTip:setDesiredPosition(x, y)
	self.desiredX = x
	self.desiredY = y
...
function ISClimaToolTip:adjustPositionToAvoidOverlap(avoidRect)
...
function ISClimaToolTip:placeAbove(r1, r2)
	local r = r1
	r.y = r2.y - r.height - 8
	r.y = math.max(0, r.y)
	return r
...
function ISClimaToolTip:setOwner(ui)
	self.owner = ui
end
